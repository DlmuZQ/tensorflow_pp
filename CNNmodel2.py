...
from read_CNN_data import read_all_train_data,read_test_data,read_train_data_split
...
def test_cnn(x_data):
    ...
    with tf.Session() as sess: 
        ...
        saver.restore(sess,"./model_3/cnn.model-22500")
        ...
        '''
        for i in range(len(x_data)//398):
            x_in = x_data[i*398:(i+1)*398]
            x_in = np.array(x_in)
            label = sess.run(preject,feed_dict={X:x_in,keep_prob:1})
            l.append(label)            
        #print(label)
...
if __name__ == '__main__':
    ...
    if train == 1:
        ...
        x_data,y_data = read_train_data_split()
    ...
    if train == 0:
        x_data = read_test_data('E:/Imaging/ROI_AVIRIS_test/test_roi_2_frm_envi.txt',3)
        #x_in = x_data[:24]
        #x_in = np.array(x_in)
        ...
        a = np.array(l)
        a = a.reshape(1,158404)
        save_viaris_img_2(a,'test_roi_2_cnn.jpg',398,398)